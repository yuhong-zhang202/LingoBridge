...
- **卡片主角化**：`FeedbackCard` 居中占舞台中心；保留身后的**叠卡** peek（`rotate(2.5deg) scale(0.96)`，还有下一张时显示），暗示"还有卡在后面"。卡片宽度收在 `max-w-[400px]` 居中，避免被拉到 600 显得空（实现最终收在 400px：460 在实机上略空，400 更聚焦，此为有意值）。
