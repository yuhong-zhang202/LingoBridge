...
| 匹配整跑单飞 | `src/lib/matching-inflight.ts` 进程内在飞表 | ⚠️ **仍有效、但只挡得住本实例内的并发** —— 同一条语料的两个并发请求落到不同实例时各跑各的，退回改动前的水位（多花一次约 ¥0.0085 + 一份并发占用）。**无正确性风险**：它去掉的只是重复的模型调用，每个请求自己的鉴权/同意/额度计次/熔断/记账本就各走各的。可以先扩容再优化，不是硬阻塞项 |
