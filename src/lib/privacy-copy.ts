 ...
 *
 * ⚠️ 2026-08-06 新增「你的数据存放在哪里」一节（跨境存储披露），**但刻意没有 bump 版本号**：
 *   产品方定的是「放进隐私详情页供查看」，不弹窗打扰老用户。代价必须说清楚 ——
 *   `BETA_PRIVACY_VERSION` 同时是 consent_records 落库的 consent_version，不 bump 就意味着
 *   **已签的 125 名用户，其同意记录对应的仍是不含跨境披露的 v3**。
 *   PIPL 第 39 条要求向境外提供个人信息须取得单独同意，所以这是一笔明确记账的合规敞口，
 *   不是疏漏。要闭合它只需把 VERSION 改成 4（老用户下次进来重签一次，缓存 key 含版本号会自然失效）。
 *   **重签成本随用户数线性上涨，越早做越便宜。**
 */
