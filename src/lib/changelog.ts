...
export const CHANGELOG: ChangelogEntry[] = [
  {
    ...
    // 跳 0.11.0 而非 0.10.10：这次是流程结构改变（分析与练习由串联改并联、文字路径少一页），不是修补。
    version: 'v0.11.0',
    date: '2026-09-01',
    title: '想直接开口练，不用先看分析了',
    notes: [
      '匹配到题目后，「题目分析」和「开始练习」变成并排的两个入口：想先理清思路就点分析，想直接开口说就点练习，顺序你自己定',
      '打字讲故事的话，写完直接进下一步，不用再经过整理确认那一页；录音仍然会先让你看一眼转写稿，可以改',
      '匹配页的题目卡不再支持右滑存题卡，改点卡片右上角的书签图标——功能没变，只是入口收成了一个',
    ],
  },
  {
    version: 'v0.10.9',
