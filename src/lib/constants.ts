...
/** B 类低相关兜底：最多展示相关性最高的前几道（2026-08-03 产品方定稿由 5 收到 4：
 *  这几道是「确实翻遍题库了」的佐证而非备选，列四道足以取信，第五道只是拉长页面） */
export const LOW_MATCH_SHOW_MAX = 4
...
/**
 * 匿名【终身】转写次数上限（跨所有自然日累计，不随每日重置）。
 *
 * 为什么只有转写有终身闸：豆包 ASR 占匿名成本的 94~97%（¥0.003/秒），是给得最多也最贵的一项；
 * 其余匿名额度（polish/practice/analysis/…）加起来不到成本的 6%，不值得为它们加一道闸。
 * 没有这道闸时，匿名账号建完 1 条语料后可以从素材库反复进练习页，每天领一份新的每日额度、无限期。
 *
 * 取值 25 的来由：录 1 个故事 = 1 次转写；一场练习最多 8 轮、每轮说一句转写一次 = 8 次；
 * 故「完整链路 + 3 场练习」≈ 1 + 3×8 = 25 次，足够体验完整产品闭环。
 * 成本上界 ≈ 25 × 约 44 秒 × ¥0.003/秒 ≈ 每个匿名账号终身最多 ¥3.3（此前无上界）。
 *
 * ⚠️ 与 ANON_TRANSCRIBE_LIMIT 数值相同（都是 25），于是每日那道闸在匿名侧不会再单独触发 ——
 *    这是【刻意保留的冗余】，不是重复代码：每日闸是限速器（挡单日突发），终身闸是总量闸。
 *    将来若把终身值调高（如 40），每日 25 的限速作用立刻恢复。两道闸都不要删。
 */
export const ANON_TRANSCRIBE_LIFETIME = 25
...
// ── 全局预算熔断（唯一一条与攻击手段无关的止损）─────────────────────────────
/**
 * 全站单日 AI 花费熔断线（人民币元，东八区日界）。今日总花费触线后，**匿名会话**的新 AI 调用一律拒绝；
 * 注册用户不受影响。判定与接线见 src/lib/global-budget-breaker.ts。
 *
 * 【与 dashboard-shared 的 DAILY_BUDGET_CNY(=20) 是两个东西，刻意不复用】
 *   · DAILY_BUDGET_CNY —— 看板趋势图上的一条**参照线**，超了只是染红，不改变任何行为；
 *   · 本常量           —— **执行闸**，触线即拒绝匿名请求。
 *   两者语义不同、将来会各自调整（参照线跟着预期用量走，熔断线跟着「亏多少钱能接受」走），
 *   合成一个常量会让「调看板配色」这种小事变成「改止损线」这种大事。
 *
 * 【取值 60 的来由】= 日预算参照线 ¥20 的 3 倍（2026-08-12 审计 P0-2 建议值，产品方拍板）。
 * 留 3 倍余量是因为它拦的是**匿名**：正常经营日远达不到（实测匿名 14 天合计仅 ¥4.68），
 * 而真被刷起来时，¥60 是「一天最多亏这么多」的上界。压到 ¥20 会在正常业务放量时误伤真实新用户。
 *
 * ⚠️ 这道闸【挡不住】注册用户（注册用户的护栏是各 REG_*_DAILY_LIMIT 与人工封号），
 *    也不是精确到分的硬止损：读数带 TTL 缓存，触线到停住之间有一个窗口（上界估算见 breaker 模块顶注）。
 */
export const GLOBAL_DAILY_BUDGET_BREAKER_CNY = 60
