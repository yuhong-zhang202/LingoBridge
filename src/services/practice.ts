...
- 需要优化：{ "needsWork": true, "optimized": "改进后的英文句子", "note": "中文点明问题在哪+怎么改，短而准，最多两句" }
...
- note 写法（针对性要强、但短而准，最多两句，别占太多空间）：用中文点清「问题在哪 + 怎么改」，按此优先级点名——
  ① 有语法错误 → 指出错在哪（时态/单复数/缺动词/搭配等），点明原句的错处并给出对的形式；
  ② 有词/词组不地道或中式表达 → 点名原词、给更自然的说法、一句话说清为什么（如「make a decision 比 do a decision 自然」）；
  ③ 语法和用词都没问题、只是能更自然 → 直接说提升点在哪。
  语法最多点 1 条、词组最多点 1 条，别罗列一堆撑长。
  只写真实发生的改动：某个词在 optimized 里没变，就绝不写「X → X」这种没改动的空账。
  别为了点问题而硬找问题：没有语法错误就别编一个出来；这句本就够好（回到上面第一步的判断），就返回 needsWork=false、不硬凑 note。
