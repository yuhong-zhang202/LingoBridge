...
export default function OfflineState({ onRetry }: { onRetry: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center text-center px-8 py-16">
      ...
      <h2 className="text-[17px] font-semibold text-v2-text-primary mt-5">网络好像断开了</h2>
      <p className="text-[13px] text-v2-text-secondary leading-relaxed text-center mt-2.5">
        请检查网络连接后重试。你已保存的故事和语料不会丢失。
      </p>
      <GradientButton
        onClick={onRetry}
        className="mt-6 px-6 py-3 rounded-full text-[14px] font-medium flex items-center gap-1.5"
      >
        <RotateCw size={15} />
        重试
      </GradientButton>
