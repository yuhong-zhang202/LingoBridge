    ...
    <Card variant="gradient" className="relative px-5 pt-4 pb-14">
      ...
      {isEditing ? (
        <textarea
          ...
          className="w-full min-h-[110px] text-[1rem] text-v2-text-primary leading-relaxed bg-transparent rounded-[12px] outline-none resize-none"
          autoFocus
        />
      ) : (
        <p className={`text-[1rem] text-v2-text-primary leading-relaxed ${canSaveAnki ? 'pr-10' : ''}`}>{text}</p>
      )}
      ...
      >
        ...
        <Card className="px-5 pt-4 pb-5">
          <div className="flex items-center gap-1.5 mb-2.5">
            ...
            <span className="text-[0.6875rem] font-medium text-v2-text-muted">素材录入</span>
          </div>
          <p className="text-[0.875rem] text-v2-text-secondary leading-relaxed">{rawStory}</p>
        </Card>
        ...
        <div className="flex items-center gap-2 px-1">
          ...
          <span className="text-[0.6875rem] text-v2-text-muted">语料梳理</span>
          <div className="flex-1 h-px bg-bg-muted" />
        </div>
        ...
        {isLoading ? (
          <div className="flex flex-col items-center py-6 gap-3">
            <Orb size={120} />
            <p className="text-[0.8125rem] text-v2-text-muted">AI 整理中…</p>
          </div>
        ) : error ? (
          <div className="flex flex-col items-center py-6 gap-3">
          ...
          <>
            ...
            />
            {usable === false && (
              <p className="text-[0.75rem] text-v2-text-muted text-center px-2 leading-relaxed">
          ...
          <div>
            <div className="px-0.5 mt-[18px]">
              <div className="flex items-center gap-1.5 mb-2.5">
                ...
                <span className="text-[0.8125rem] font-medium text-v2-text-secondary">怎样的素材更好用</span>
              </div>
              <div className="flex items-start gap-2.5 mb-2">
                <span className="flex-shrink-0 w-[18px] h-[18px] rounded-full border border-brand-primary-light bg-phrase-warm-bg flex items-center justify-center text-[0.6875rem] text-brand-primary-dark mt-[1px]">1</span>
                <p className="text-[0.8125rem] text-v2-text-secondary leading-relaxed">同一段真实经历，细节越全，能套用的题越多</p>
              </div>
              <div className="flex items-start gap-2.5">
                <span className="flex-shrink-0 w-[18px] h-[18px] rounded-full border border-brand-primary-light bg-phrase-warm-bg flex items-center justify-center text-[0.6875rem] text-brand-primary-dark mt-[1px]">2</span>
                <p className="text-[0.8125rem] text-v2-text-secondary leading-relaxed">好用的素材通常会带到：时间、人物、发生的事、你的做法和感受</p>
              </div>
            </div>

            <div className="pt-1">
              {saveError && (
                <p className="text-[0.75rem] text-red-400 text-center mb-2">{saveError}</p>
              )}
              <GradientButton
                onClick={onMatch}
                loading={isSaving}
