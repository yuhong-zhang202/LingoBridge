/**
 * @module   OrbMini
 * @desc     轻量 Orb 光晕 — 4 层径向渐变，无粒子 rAF，用于底栏「云团」按钮
 * @author   LingoBridge
 * @created  2026-05-31
 */
'use client'
...
/**
 * 无粒子静态 Orb，复用 4 层 radial-gradient，省略 42 粒子 rAF 循环
 * @param size      容器边长（px），默认 42
 * @param className 附加 class
 */
export default function OrbMini({ size = 42, className }: OrbMiniProps): JSX.Element {
  const s = size / 300

  return (
    <div
      className={className}
      style={{ width: size, height: size, position: 'relative', flexShrink: 0, overflow: 'hidden', borderRadius: '50%' }}
    >
      {/* 绿色 左上 */}
      <div
