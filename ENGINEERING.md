...
> 📋 **新会话先读 `docs/交接-最新.md`** —— 最新交接稿【活文档，只更新不新建】（现状一页纸、各批次改动、
> **产品方明确砍掉的项及理由**、仍 open 的事、沉淀的教训）。看它能省掉重复提案。
>
> 再按需读这几份**活文档**（持续更新，以它们为准）：
> - `docs/未完成工作-上线路线图.md` —— 待办活台账
> - `docs/部署交接-香港PaaS.md` —— 部署步骤 + 环境变量 + 运维铁律
> - `docs/排期-全面审计发现-2026-08-06.md` —— 审计 P0/P1/P2/P3 分级清单
> - `docs/匹配链路-权威文档.md`、`docs/PRODUCT_INVARIANTS.md` —— AI 匹配架构与产品红线
> - `CLAUDE.md` —— 协作约定（语言 / 派角色 / 验证纪律）
> - `scripts/eval/loop/DISCUSSION_LOG.md` —— 议题台账（各文档里的编号引用指向它）
>
> 历史留档 `docs/交接-统一交接稿-2026-07-25.md`：原 `交接文档/` 下 21 份历史稿的收敛版。
> ⚠️ 停在 2026-07-25（彼时尚处「内测前收尾」，现已上线运营），**仅供回溯背景，不可当现状读**。
...
**「因为 X 所以取值 Y」的注释，必须核对 X 和 Y 真在同一个作用域里**（2026-08-06 审计沉淀）：

凡是注释写了某个常量的取值依据，改动或复查时都要单独确认那条依据现在还成立、且**它保护的东西真的在这段代码的管辖范围内**。

真实案例：转写接口的并发闸有两段注释，各自都写得很认真、还互相引用：

```ts
// 注释 A（解释为什么并发数是 2）
// 生产是 2vCPU，ffmpeg 转码是 CPU 密集型，并发超过核数会互抢 CPU，故压到 2

// 注释 B（解释闸包住什么）
// 并发闸只包住豆包调用这一段。转码是本机 CPU、不占豆包配额，绝不能圈进来
```

两句单独看都对，**叠在一起就是错的**：闸压到 2 是为了保护 CPU，而闸恰恰不包含那段吃 CPU 的转码。结果是转码并发实际无上限，而 ASR 吞吐白白砍半。

这类错误编译器和测试都抓不到，只能靠复查时问一句：**这个理由保护的对象，真的在这个闸的管辖范围内吗。**

注释质量越高的代码库越容易栽在这上面，因为读的人会不自觉地信任注释而不去核对代码。
...
- 判分标准（喂给模型的 rubric）：不用改故事即可回答 ≥85（高）；同故事换角度/挪重心/习惯套单次 60–84（中）；必须换故事 30–59（低）；<30 答非所问
- 展示语义（代码两条线，2026-07-16 起）：**≥85 首屏 / [60,85) 折叠 / <60 一律不展示不入库**。
  模型的「低」与「答非所问」在产品侧合并为同一件事：不给用户看
- 降级策略：模型调用失败静默返回空数组，调用方保留漏斗排序原序展示

**分档阈值常量**（`src/lib/constants.ts`，改阈值只改这里）
```ts
export const SCORE_HIGH = 85   // 高匹配：首屏直接展示
export const SCORE_MID  = 60   // 中匹配：折叠进「查看更多」。低于此值：不展示、不入库
```
**只有两条线。** 原 `SCORE_LOW=40` 划出的「低匹配折叠可见（40-59）」档已于 2026-07-16
由产品方拍板取消（台账 042）。依据是实测——**低档在模型输出里是假精度**：40-59 区间的
21 条候选全部堆在 40 这一个点上，题目显示与否取决于模型凑整到 40 还是 35。

> **035 闭合**：此前 prompt 告诉模型「30-59=低」，而代码的可见线是 40 —— 30-39 这一带
> 模型以为自己在打低匹配、代码却按隐藏处理，两把尺不同数。现在可见线 = 60，与 prompt
> 的中/低分界线**首次同数**：模型判「低」的，产品侧一律不展示，不再有分歧带。
